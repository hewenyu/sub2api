...
export const Dashboard: React.FC = () => {
  ...
  } = useUsage(undefined, true, 30000);
  const {
    status: accountStatus,
    isLoading: accountsLoading,
    error: accountsError,
    refetch: refetchAccounts,
  } = useAccounts(true, 30000);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Welcome to Claude Relay Admin Dashboard</p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <UsageChart data={summary?.daily_usage || []} isLoading={usageLoading} />

        <ModelDistributionChart isLoading={usageLoading} />
      </div>

      <AccountStatus status={accountStatus} isLoading={accountsLoading} />

      <RecentRequests requests={recentRequests} isLoading={usageLoading} />
    </div>
  );
};
