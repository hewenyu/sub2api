...
export const apiKeyApi = {
  list: (params?: PaginationParams) =>
    request.get<PaginatedResponse<APIKey>>('/api-keys', params),

  get: (id: number) =>
    request.get<APIKey>(`/api-keys/${id}`),

  create: (data: CreateAPIKeyRequest) =>
    request.post<APIKey>('/api-keys', data),

  update: (id: number, data: UpdateAPIKeyRequest) =>
    request.put<APIKey>(`/api-keys/${id}`, data),
...
export const codexAccountApi = {
  ...
  get: (id: number) =>
    request.get<CodexAccount>(`/codex-accounts/${id}`),

  create: (data: CreateCodexAccountRequest) =>
    request.post<CodexAccount>('/codex-accounts', data),

  update: (id: number, data: UpdateCodexAccountRequest) =>
    request.put<CodexAccount>(`/codex-accounts/${id}`, data),

  delete: (id: number) =>
    request.delete(`/codex-accounts/${id}`),
};
...
export const usageRecordApi = {
  ...
  get: (id: number) =>
    request.get<UsageRecord>(`/usage-records/${id}`),
};

export const adminApi = {
  list: (params?: PaginationParams) =>
    request.get<PaginatedResponse<Admin>>('/admins', params),
